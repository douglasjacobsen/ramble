...
def colorize_spec(spec):
    ...
    class insert_color:
        ...
        def __call__(self, match):
            ...
            return f"{color_formats[sep]}{clr.cescape(sep)}"
...
class _EdgeMap(Mapping):
    ...
    def _cmp_iter(self):
        yield from sorted(itertools.chain.from_iterable(self.edges.values()))
...
class ForwardQueryToPackage:
    ...
    def __get__(self, instance, cls):
        ...
        if query.isvirtual:
            specialized_name = "{}_{}".format(query.name, self.attribute_name)
...
class Spec:
    ...
    def traverse_edges(self, visited=None, d=0, deptype="all", dep_spec=None, **kwargs):
        ...
        if not (key in visited and cover == "edges"):
            ...
            for dspec in sorted(edges(), key=key_fn):
                ...
                yield from succ(dspec).traverse_edges(visited, d + 1, deptype, dspec, **kwargs)
    ...
    def dependencies_from_node_dict(node):
        ...
        if "dependencies" not in node:
            return
        yield from Spec.read_yaml_dep_specs(node["dependencies"])
    ...
    def validate_detection(self):
        ...
        msg = 'cannot validate "{}" since it was not created ' "using Spec.from_detection".format(
            self
        )
    ...
    def _merge_dependency(self, dependency, visited, spec_deps, provider_index, tests):
        ...
        if dep.name not in spec_deps:
            ...
            try:
                tty.debug(f"{self.name} applying constraint {str(dep)}")
                changed |= spec_deps[dep.name].constrain(dep)
            except spack.error.UnsatisfiableSpecError as e:
                fmt = f"An unsatisfiable {e.constraint_type}"
                fmt += " constraint has been detected for spec:"
                fmt += f"\n\n{spec_deps[dep.name].tree(indent=4)}\n\n"
    ...
    def dep_difference(self, other):
        ...
        mine = {s.name for s in self.traverse(root=False)}
    ...
    def old_format(self, format_string="$_$@$%@+$+$=", **kwargs):
        ...
        token_transforms = {k.upper(): v for k, v in kwargs.get("transform", {}).items()}
    ...
    def splice(self, other, transitive):
        ...
        if virtuals_to_replace:
            deps_to_replace = {self[v]: other for v in virtuals_to_replace}
        ...
        for d in deps_to_replace:
            if not all(
                ...
                raise SpliceError(
                    ("Splice between {} and {} will not provide " "the same virtuals.").format(
                        self.name, other.name
                    )
                )
        ...
        if transitive:
            other_nodes = {s.name: s.copy(deps=False) for s in other.traverse(root=True)}
        else:
...
class SpecParser(spack.parse.Parser):
    ...
    def spec_from_file(self):
        ...
        if not (path.endswith(".yaml") or path.endswith(".json")):
            raise SpecFilenameError(f"Spec filename must end in .yaml or .json: '{path}'")

        if not os.path.exists(path):
            raise NoSuchSpecFileError(f"No such spec file: '{path}'")
...
def save_dependency_specfiles(
    ...
    if spec_format == "json":
        ...
        raise SpecParseError("Unrecognized spec format {}.".format(spec_format))
    ...
    for dep_name in dep_list:
        if dep_name not in root_spec:
            msg = "Dependency {} does not exist in root spec {}".format(dep_name, root_spec.name)
            raise SpecDependencyNotFoundError(msg)
        dep_spec = root_spec[dep_name]
        json_path = os.path.join(output_directory, f"{dep_name}.json")
...
class InvalidDependencyError(spack.error.SpecError):
    ...
    def __init__(self, pkg, deps):
        ...
        super().__init__(
            "Package {} does not depend on {}".format(pkg, spack.util.string.comma_or(deps))
        )
...
class InvalidHashError(spack.error.SpecError):
    def __init__(self, spec, hash):
        super().__init__("The spec specified by %s does not match provided spec %s" % (hash, spec))


class NoSuchHashError(spack.error.SpecError):
    def __init__(self, hash):
        super().__init__("No installed spec matches the hash: '%s'" % hash)
...
class ConflictsInSpecError(spack.error.SpecError, RuntimeError):
    def __init__(self, spec, matches):
        message = 'Conflicts in concretized spec "{}"\n'.format(spec.short_spec)
