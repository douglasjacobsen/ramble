...
class Renderer:
    def render_objects(self, render_group, exclude_where=None, ignore_used=True, fatal=True):
        ...
        if matrices:
            ...
            matrix_product_iters = [
                itertools.product(*vectors) for vectors in matrix_vectors
            ]
                ...
                for entry_tuple in zip(*matrix_product_iters):
                    ...
                    for mat_idx, entry in enumerate(entry_tuple):
                        ...
                        for name_idx, val in enumerate(entry):
                            ...
                            # if var in object_variables: vectors.append(object_variables[var]) -> values
        ...
        for var, val in object_variables.items():
            if isinstance(val, list) and (
                var in used_variables or not ignore_used
            ):
        ...
        if vector_vars:
            ...
            matrix_objects = (
                list(matrix_generator) if matrix_generator else None
            )
        ...
        for obj in new_objects:
            ...
            if exclude_where:
                for where in exclude_where:
                    evaluated = where_expander.expand_var(
                        where, extra_vars=obj
                    )
                    if evaluated == "True":
                        keep_object = False
                        break
